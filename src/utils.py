...
class Utils:
    ...
    def getDailySetPromotions(self) -> list[dict]:
        ...
        date_formats = [
            today.strftime("%m/%d/%Y"),  # 04/19/2025
            today.strftime("%#m/%#d/%Y"),  # 4/19/2025 (removing leading zeros)
            today.strftime("%Y-%m-%d"),  # 2025-04-19
            today.strftime("%d/%m/%Y"),  # 19/04/2025
        ]

        for date_format in date_formats:
            if date_format in dashboard_data["dailySetPromotions"]:
                return dashboard_data["dailySetPromotions"][date_format]

        # If no matching date format is found, log the available keys
        available_keys = list(dashboard_data["dailySetPromotions"].keys())
        logging.warning(
            f"No matching date format found. Available date keys: {available_keys}"
        )

        # If no keys match, return an empty list to avoid crashing
        if available_keys:
            # If there are keys, return the first one (most recent data)
            return dashboard_data["dailySetPromotions"][available_keys[0]]
